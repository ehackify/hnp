...
cat > /etc/logstash/conf.d/hnp.conf <<EOF
input {
  file {
    path => "/var/log/hnp/hnp-json.log"
  }
}
...
output {
  elasticsearch {
    ...
    index => "hnp-%{+YYYYMMddHH00}"
    template_name => "hnp_event"
    template => "/etc/logstash/conf.d/hnp-template.json"
...
cat > /etc/logstash/conf.d/hnp-template.json <<EOF
{
  "index_patterns": "hnp-*",
  "settings": {
    "number_of_shards": 5,
