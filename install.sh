...
chmod +x /opt/hnp/scripts/*
set -e
set -x

hnp_HOME=`dirname "$(readlink -f "$0")"`
WWW_OWNER="www-data"
SCRIPTS="$hnp_HOME/scripts/"
cd $SCRIPTS
...
echo "[`date`] Starting Installation of all hnp packages"
...
echo "[`date`] ========= Installing hnp Server ========="
 ./install_hnpserver.sh

echo "[`date`] ========= hnp Server Install Finished ========="
echo ""
...
do
    ...
    then
        ...
        read SPLUNK_PORT
        echo "The Splunk Universal Forwarder will send all hnp logs to $SPLUNK_HOST:$SPLUNK_PORT"
...
while true;
do
    echo -n "A properly configured firewall is highly encouraged while running hnp."
    echo -n "This script can enable and configure UFW for use with hnp."
    echo -n "Would you like to add hnp rules to UFW? (y/n) "
    read UFW
    if [ "$UFW" == "y" -o "$UFW" == "Y" ]
    then
        ./enable_ufw.sh
...
chown $WWW_OWNER /var/log/hnp/hnp.log

chown $WWW_OWNER /var/log/hnp/hnp.log
supervisorctl restart hnp-celery-worker

echo "[`date`] Completed Installation of all hnp packages"
