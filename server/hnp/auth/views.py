...
from hnp import db, mail
from hnp import user_datastore
from hnp.common.utils import error_response
from hnp.auth.models import User, PasswdReset, ApiKey
from hnp.auth import errors
from hnp.auth import (
    get_datastore, login_required, roles_accepted, current_user)
from hnp.api import errors as apierrors
import uuid
...
def reset_passwd_request():
    ...
    from hnp import hnp
    msg = Message(
            html=reset.email_body, subject='hnp Password reset',
            recipients=[user.email], sender=hnp.config['DEFAULT_MAIL_SENDER'])
    try:
        mail.send(msg)
    except:
