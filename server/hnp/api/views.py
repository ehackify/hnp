...
from hnp import db, csrf
from hnp.api import errors
from hnp.api.models import (
        Sensor, Rule, DeployScript as Script,
        DeployScript, RuleSource)
from hnp.api.decorators import deploy_auth, sensor_auth, token_auth
from hnp.common.utils import error_response
from hnp.common.clio import Clio
from hnp.auth import current_user, login_required
...
def get_rules():
    ...
    if request.args.get('plaintext') in ['1', 'true']:
        ...
        resp.headers['Content-Disposition'] = "attachment; filename=hnp.rules"
        return resp
    else:
